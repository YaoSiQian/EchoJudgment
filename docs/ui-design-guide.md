...
| 属性 | 值 |
|------|-----|
| 尺寸 | 375×812px（iPhone 14 比例）|
| 外框 | `border-radius: 52px`，多层 box-shadow 模拟真实手机 |
| 状态栏 | 高度 50px，白色背景，Dynamic Island 黑色胶囊（120×34px）|
| 应用视口 | `border-radius: 0 0 48px 48px`（底部圆角跟随机身）|
| 图标 | Signal / WiFi / Battery 均为 SVG，颜色 `#111` |
...
> **实现注意**：使用 SVG outline 图标而非 emoji，保证清晰度与 XHS 视觉一致性。active 状态描边加粗（strokeWidth 2）并添加 8% 透明度背景填充。
...
│                               │
│  ─────────────────────────── │
│  [评价按钮区 - 契机时滑入]     │  ← EvaluationBar（覆盖输入区）
│  ─────────────────────────── │
│  😊  [说点什么…]        ➤/⊕  │  ← 输入区（有内容→发送图标，无→+）
└───────────────────────────────┘
```

> 输入区功能说明：玩家输入消息后，内容通过 `addPlayerContext()` 注入 AI 上下文，并触发 `generateNextMessage()` 生成 NPC 动态回应。评价栏出现时输入区隐藏。
